...
local function single_target ()
  ...
  if S.EnergizingElixir:IsReady() and Player:Chi() <= 1 and AC.CombatTime() > 8 and (S.RisingSunKick:CooldownUp() or (S.StrikeOfTheWindlord:IsAvailable() and S.StrikeOfTheWindlord:CooldownUp()) or Player:Energy() < 50 or
  (Cache.EnemiesCount[8] >= 6 and Player:Chi() < 3 and not Player:PrevGCD(1, S.SpinningCraneKick))) then
  ...
  if S.RisingSunKick:IsReady() and ((Player:Chi() >= 3 and Player:Energy() >= 40) or Player:Chi() >= 5) and (not S.Serenity:IsAvailable() or S.Serenity:CooldownRemains() >= 6) then
  ...
  if S.FistsOfFury:IsReady() and S.Serenity:IsAvailable() and not I.DrinkingHornCover:IsEquipped() and (S.Serenity:CooldownRemains() >= 15 or S.Serenity:CooldownRemains() <= 4) and Player:EnergyTimeToMax() > 2 then
    if AR.Cast(S.FistsOfFury) then return ""; end
  end
  ...
  if AR.AoEON() and S.SpinningCraneKick:IsReady() and Cache.EnemiesCount[8] >= 3 and not Player:PrevGCD(1, S.SpinningCraneKick) and GetSpellCount("Spinning Crane Kick") >= 2 then
...
local function serenity ()
  ...
  if S.StrikeOfTheWindlord:IsReady() or S.StrikeOfTheWindlord:CooldownRemains() < Player:GCD() / 3 then
    if AR.Cast(S.StrikeOfTheWindlord) then return ""; end
  end
...
local function APL ()
  ...
  if Everyone.TargetIsValid() then
    ...
    if not S.Serenity:IsAvailable() and I.DrinkingHornCover:IsEquipped() and (S.StrikeOfTheWindlord:CooldownRemains() <= 18 and S.FistsOfFury:CooldownRemains() <= 12 and Player:Chi() >= 3
    and S.RisingSunKick:CooldownRemains() <= 1 or Target:TimeToDie() <= 25 or S.TouchOfDeath:CooldownRemains() > 112) and S.StormEarthAndFire:Charges() == 1 then
      ShouldReturn = sef();
      if ShouldReturn then return ShouldReturn; end
    end
    -- actions+=/call_action_list,name=sef,if=!talent.serenity.enabled&!equipped.drinking_horn_cover&(cooldown.strike_of_the_windlord.remains<=14&cooldown.fists_of_fury.remains<=6&chi>=3&
    -- cooldown.rising_sun_kick.remains<=1|target.time_to_die<=15|cooldown.touch_of_death.remains>112)&cooldown.storm_earth_and_fire.charges=1
    if not S.Serenity:IsAvailable() and not I.DrinkingHornCover:IsEquipped() and (S.StrikeOfTheWindlord:CooldownRemains() <= 14 and S.FistsOfFury:CooldownRemains() <= 6 and Player:Chi() >= 3 and
    S.RisingSunKick:CooldownRemains() <= 1 or Target:TimeToDie() <= 15 or S.TouchOfDeath:CooldownRemains() > 112) and S.StormEarthAndFire:Charges() == 1 then
      ShouldReturn = sef();
      if ShouldReturn then return ShouldReturn; end
    end
    ...
    if not S.TigerPalm:IsUsable() and not Player:PrevGCD(1, S.TigerPalm) and Player:Chi() < 2 then
      if AR.Cast(S.TigerPalm) then return ""; end
    end
    -- actions.st+=/rising_sun_kick
    if S.RisingSunKick:IsUsable() then
      if AR.Cast(S.RisingSunKick) then return ""; end
    end
    -- actions.st+=/chi_wave
    if S.ChiWave:IsReady() and (S.ChiWave:CooldownRemains() < Player:EnergyTimeToX(50, 0) or Player:PrevGCD(1, S.TigerPalm) then
      if AR.Cast(S.ChiWave) then return ""; end
    end
    -- actions.st+=/chi_burst
    if S.ChiBurst:IsReady() and (S.ChiBurst:CooldownRemains() < Player:EnergyTimeToX(50, 0) or Player:PrevGCD(1, S.TigerPalm) then
      if AR.Cast(S.ChiBurst) then return ""; end
    end
    -- actions.st+=/tiger_palm
    if not S.TigerPalm:IsUsable() and not Player:PrevGCD(1, S.TigerPalm) then
      if AR.Cast(S.TigerPalm) then return ""; end
    end
    return;
  end
